...
public class HandEvaluator {
	...
	public static void main(String args[]) {
		...
		System.out.println("constructor ="+(t - b));
		
		for (b1 = 0; b1 < 1; b1++) {
			board.addCard(b1);
			for (b2 = b1+1; b2 < 49; b2++) {
				board.addCard(b2);
				for (b3 = b2+1; b3 < 50; b3++) {
					board.addCard(b3);
					for (b4 = b3+1; b4 < 51; b4++) {
						board.addCard(b4);
						for (b5 = b4+1; b5 < 52; b5++) {
							board.addCard(b5);
							ranks = handEval.getRanks(board);
							board.removeCard();							
						}
						board.removeCard();						
					}
				...
				t = System.currentTimeMillis() / 1000;
				System.out.println("get Ranks ="+(t - b)+", b3="+b3);
				board.removeCard();				
			}
			board.removeCard();
		}
		...
		t = System.currentTimeMillis() / 1000;
		System.out.println("get Ranks ="+(t - b));
	}
	...
	// not_allowec:\\pokerprog Card.NUM_RANKS means any
	// returns ident value
	...
	public final static int rankHand(Hand h) {
		...
		// pair data
		byte[] group_size = new byte[POKER_HAND + 1]; // array to track the
														// groups or cards in
														// your hand
		byte[] paired = new byte[Card.NUM_RANKS]; // array to track paired carsd
		byte[][] pair_rank = new byte[POKER_HAND + 1][2]; // array to track the
															// rank of our pairs
		// straight
		...
		for (r = 0; r < h.size(); r++) {
			...
			paired[rank]++; // Add rank of card to paired array to track the
							// pairs we have.
			group_size[paired[rank]]++; // keep track of the groups in our hand
										// (1-pair, 2-pair, 1-trips, 1-trips
										// 1-pair)
			if (paired[rank] != 0) // To prevent looking at group_size[-1],
									// which would be bad.
				group_size[paired[rank] - 1]--; // Decrese the previous group by
												// one. group_size[0] should end
												// up at -5.
			if ((++suit_size[suit]) >= POKER_HAND) { // Add suit to suit array,
														// then check for a
														// flush.
				flush = true;
				major_suit = suit;
		...
		for (r = 0; r < Card.NUM_RANKS; r++) {
			// check straight
			if (paired[r] != 0) {
				if ((++straight_size) >= POKER_HAND) { // Do we have five cards
														// in a row (a
														// straight!)
		...
		if (group_size[POKER_HAND] != 0) { // we have five cards of the same
											// rank in our hand.
		...
		if (group_size[4] != 0) {
			...
			pair_rank[4][1] = (byte) Card.NUM_RANKS; // just in case 2 sets
														// quads
	...
	private static String name_hand(int rank) {
		...
		case THREEKIND:
			t = "Three of a Kind, "
					+ rank_name[ident / (NUM_RANKS * NUM_RANKS)] + "s";
			break;
		case FULLHOUSE:
